...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    (mles.gamma <- optim(par = c(1,1),    ##how do I know what to guess here?
             fn = llgamma,
             data=dat.wind$speed,      #### WHAT DATA SHOULD I PUT IN????
             neg=T))
    alpha.hat.mle <- mles.gamma$par[1]
    beta.hat.mle <- mles.gamma$par[2]
    @
    \item Compute the MLEs for these data using the Log-Normal distribution.
    <<>>=
    ll_lognorm <- function(data, par, neg=F){
      mu <- par[1]
      sigma <- par[2]
  
      loglik <- sum(dlnorm(x=data, meanlog=mu, sdlog=sigma, log=TRUE))
  
      return(ifelse(neg, -loglik, loglik))
      }
    
    (mles.ll <- optim(par = c(0,1),  ##how do I know what to guess here?
                   fn = ll_lognorm,
                   data = dat.wind$speed,  #### WHAT DATA SHOULD I PUT IN????
                   neg = TRUE))

    mu.hat.mle <- mles.ll$par[1]
    sigma.hat.mle <- mles.ll$par[2]
    @
    ...
    <<>>=
    # likelihood weibull / likelihood gamma
    loglikely.gamma <- -mles.gamma$value
    gamma.weibull.ratio <- 2166.496/ loglikely.gamma
    ...
    <<>>=
    # likelihood weibull/ likelihood lognormal
    loglikely.ll <- -mles.ll$value
    lognormal.weibull.ratio <- 2166.496/ loglikely.ll
    ...
    <<>>=
    gamma.lognormal.ratio <- loglikely.gamma/ loglikely.ll
